...
# Response format filter messages
FORMAT_NOTE_IMAGE_NO_IMAGE = "\n\n(Note: Response format set to 'image', but no image was generated)"
FORMAT_NOTE_BOTH_NO_IMAGE = "\n\n(Note: Response format set to 'both', but no image was generated)"
FORMAT_NOTE_BOTH_NO_TEXT = "(Image generated without text description)"
...
def apply_response_format_filter(response_text, images_received, response_format):
    ...
    This function is called after plugin hooks have processed the response,
    allowing plugins to add/modify content before format filtering is applied.
    Any content generated by plugins may be filtered out based on user preference.
    ...
    if response_format == RESPONSE_FORMAT_AUTO:
        ...
        if images_received:
            ...
            return response_text + FORMAT_NOTE_IMAGE_NO_IMAGE, []
        ...
        if not response_text and images_received:
            return FORMAT_NOTE_BOTH_NO_TEXT, images_received
        elif response_text and not images_received:
            return response_text + FORMAT_NOTE_BOTH_NO_IMAGE, []
        else:
            return response_text, images_received
    else:
...
def get_reply(message, image_data_64_list, session_id):
    ...
    # Apply response format filtering based on user preference
    # Note: This is applied AFTER plugin hooks, so any content added by plugins
    # may be filtered out according to the user's format preference (e.g., if a plugin
    # adds images but user has format set to 'text', those images will be filtered out)
        ...
        for message_group in grouped_messages:
            ...
            try:
                ...
                # Handle /format command to set response format preference
                    ...
                    # Get the requested format (with safety check)
                    if len(parts) < 2:
                        send_message(chat_id, "❌ Please specify a format option. Use /format to see available options.")
                        continue
